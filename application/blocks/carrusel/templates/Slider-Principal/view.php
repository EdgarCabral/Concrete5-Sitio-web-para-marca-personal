...
<!--<?php echo $navigationTypeText?>-->
    <?php if(count($rows) > 0) { ?>
    <div class="sliderprincipal carrusel-<?php echo $bID ?> " style="<?php if ($maxWidth) { echo 'width:'.$maxWidth.'px'; } ?> display:none">
        <?php foreach($rows as $row) { ?>
            ...
            <?php if(is_object($f)) {?>
                ...
                <div class="row" style="background-color: #1b224f; margin:0px !important">
                    <?php
	                $src = $f->getThumbnailURL('medium');
	                $style = "background-image: url('".$src."');";
	            ?>
                    <div style="padding:0px !important; <?php echo $style; ?>" class="<?php if($row['title'] || $row['description']) { echo "col l8 m12 s12"; }else{ echo "col l12"; } ?> foto_portada_slide">
                        <?php
                        /*
