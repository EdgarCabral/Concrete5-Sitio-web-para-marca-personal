<?php  
namespace Concrete\Package\WallGallerySuite\Block\WallGallery;
...
{
    protected $btTable = 'btWallGallery';
    ...
    function loaders()
    {
        ...
        $bt = BlockType::getByHandle('wall_gallery');
    ...
    function view(){
        ...
        foreach($files as $file){
            if(in_array($file->getFileID(),$existingThumbIDs)){
                $sort = $db->GetOne('SELECT sort from btWallGalleryThumb WHERE bID = ? and fID = ?', array($bID,$file->getFileID()));
    ...
    public function save($args) {
        $db = Database::connection();
        $db->executeQuery('DELETE from btWallGalleryThumb WHERE bID = ?', array($this->bID));
        $count = count($args['sort']);
        $i = 0;
        parent::save($args);
        while ($i < $count) {
            $vals = array($this->bID,$args['fID'][$i],$args['sort'][$i]);     
            $db->executeQuery('INSERT INTO btWallGalleryThumb (bID,fID, sort) values(?,?,?)', $vals);
            $i++;
        }
    ...
    public function duplicate($newBID) {
        ...
        $db = Database::connection();
        $vals = array($this->bID);
        $data = $db->query('SELECT * FROM btWallGalleryThumb WHERE bID = ?', $vals);
	...
	public function validate($args) {
        ...
        if(empty($args['pause'])){
            $e->add(t("Pause Duration must have a value."));
        }
        if(!ctype_digit(trim($args['pause']))){
            $e->add(t("Pause Duration must be numeric only."));
        }
        if(empty($args['thumbMargin'])){
            $e->add(t("Thumbnail Margins must have a value."));
        }
        if(!ctype_digit(trim($args['thumbMargin']))){
            $e->add(t("Thumbnail Margins must be numeric only."));
        }
        return $e;
    }
	
	public function registerViewAssets($outputContent = '') {
		$this->requireAsset('javascript','wallgallery');
	}
